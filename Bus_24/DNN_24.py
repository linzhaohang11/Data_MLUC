...
Set_gen = [3, 4, 7, 8, 20, 21, 30, 31, 32]
...
Data_feature = pd.read_csv(Data_feature_path)/5000
...
for i_gen in Set_gen:
    if i_gen < 10:
        dic['DNN_G0' + str(i_gen)] = MLPClassifier(max_iter = 10000, solver = 'adam', activation = 'relu')
        ...
        print('Parameters for DNN_G0',str(i_gen), ':', dic['Best_Para_G0' + str(i_gen)])
    if i_gen >= 10:
        dic['DNN_G' + str(i_gen)] = MLPClassifier(max_iter = 10000, solver = 'adam', activation = 'relu')
