...
param_grid = {'n_estimators':[5, 25, 50], 'max_depth': [3, 5, 9], 'min_samples_split': [2, 4]}
for i_gen in Set_gen:
    if i_gen < 10:
        dic['RF_G00' + str(i_gen)] = RandomForestClassifier(criterion='gini')
        dic['RF_GS_G00' + str(i_gen)] = GridSearchCV(dic['RF_G00' + str(i_gen)], param_grid, cv = 10, scoring = 'accuracy')
        dic['RF_GS_G00' + str(i_gen)].fit(Train_feature.values.T, dic['Train_label_G00' + str(i_gen)].values.T)
        dic['Best_Para_G00' + str(i_gen)] = dic['RF_GS_G00' + str(i_gen)].best_params_
        print('Parameters for RF_G00',str(i_gen), ':', dic['Best_Para_G00' + str(i_gen)])
    if 10 <= i_gen < 100:
        dic['RF_G0' + str(i_gen)] = RandomForestClassifier(criterion='gini')
        dic['RF_GS_G0' + str(i_gen)] = GridSearchCV(dic['RF_G0' + str(i_gen)], param_grid, cv = 10, scoring = 'accuracy')
        dic['RF_GS_G0' + str(i_gen)].fit(Train_feature.values.T, dic['Train_label_G0' + str(i_gen)].values.T)
        dic['Best_Para_G0' + str(i_gen)] = dic['RF_GS_G0' + str(i_gen)].best_params_
        print('Parameters for RF_G0',str(i_gen), ':',  dic['Best_Para_G0' + str(i_gen)])
    if i_gen >= 100:
        dic['RF_G' + str(i_gen)] = RandomForestClassifier(criterion='gini')
        dic['RF_GS_G' + str(i_gen)] = GridSearchCV(dic['RF_G' + str(i_gen)], param_grid, cv=10, scoring='accuracy')
        dic['RF_GS_G' + str(i_gen)].fit(Train_feature.values.T, dic['Train_label_G' + str(i_gen)].values.T)
        dic['Best_Para_G' + str(i_gen)] = dic['RF_GS_G' + str(i_gen)].best_params_
        print('Parameters for RF_G', str(i_gen), ':', dic['Best_Para_G' + str(i_gen)])

# Use the best parameters to design RFs
for i_gen in Set_gen:
    if i_gen < 10:
        dic['RF_G00' + str(i_gen)] = RandomForestClassifier(criterion         = 'gini',
                                                            n_estimators      = dic['Best_Para_G00' + str(i_gen)]['n_estimators'],
                                                            max_depth         = dic['Best_Para_G00' + str(i_gen)]['max_depth'],
                                                            min_samples_split = dic['Best_Para_G00' + str(i_gen)]['min_samples_split'])
        dic['RF_G00' + str(i_gen)] = dic['RF_G00' + str(i_gen)].fit(Train_feature.T, dic['Train_label_G00' + str(i_gen)].T)
        print(str(i_gen), dic['RF_G00' + str(i_gen)])
    if 10 <= i_gen < 100:
        dic['RF_G0' + str(i_gen)] = RandomForestClassifier(criterion         = 'gini',
                                                           n_estimators      = dic['Best_Para_G0' + str(i_gen)]['n_estimators'],
                                                           max_depth         = dic['Best_Para_G0' + str(i_gen)]['max_depth'],
                                                           min_samples_split = dic['Best_Para_G0' + str(i_gen)]['min_samples_split'])
        dic['RF_G0' + str(i_gen)] = dic['RF_G0' + str(i_gen)].fit(Train_feature.T, dic['Train_label_G0' + str(i_gen)].T)
        print(str(i_gen), dic['RF_G0' + str(i_gen)])
    if i_gen >= 100:
        dic['RF_G' + str(i_gen)] = RandomForestClassifier(criterion         = 'gini',
                                                          n_estimators      = dic['Best_Para_G' + str(i_gen)]['n_estimators'],
                                                          max_depth         = dic['Best_Para_G' + str(i_gen)]['max_depth'],
                                                          min_samples_split = dic['Best_Para_G' + str(i_gen)]['min_samples_split'])
        dic['RF_G' + str(i_gen)] = dic['RF_G' + str(i_gen)].fit(Train_feature.T, dic['Train_label_G' + str(i_gen)].T)
        print(str(i_gen), dic['RF_G' + str(i_gen)])

# Check the scores
for i_gen in Set_gen:
    if i_gen < 10:
        dic['RF_pred_G00' + str(i_gen)] = dic['RF_G00' + str(i_gen)].predict(Test_feature.T).T
        dic['RF_true_G00' + str(i_gen)] = np.array(dic['Test_label_G00' + str(i_gen)])
        print('--------------------------------------------------------------------------------------------------------')
        print('Scores for G00',str(i_gen))
        print(classification_report(dic['RF_true_G00' + str(i_gen)].flatten(), dic['RF_pred_G00' + str(i_gen)].flatten()))
        if len(np.unique(dic['RF_true_G00' + str(i_gen)])) > 1:
            print('ROC_score         ', roc_auc_score(dic['RF_true_G00'+str(i_gen)].flatten(), dic['RF_pred_G00'+str(i_gen)].flatten()))
        else:
            print('Only one class, no ROC score.')
        print('--------------------------------------------------------------------------------------------------------')
    if 10 <= i_gen < 100:
        dic['RF_pred_G0' + str(i_gen)] = dic['RF_G0' + str(i_gen)].predict(Test_feature.T).T
        dic['RF_true_G0' + str(i_gen)] = np.array(dic['Test_label_G0' + str(i_gen)])
        print('--------------------------------------------------------------------------------------------------------')
        print('Scores for G0',str(i_gen))
        print(classification_report(dic['RF_true_G0' + str(i_gen)].flatten(), dic['RF_pred_G0' + str(i_gen)].flatten()))
        if len(np.unique(dic['RF_true_G0' + str(i_gen)])) > 1:
            print('ROC_score         ', roc_auc_score(dic['RF_true_G0'+str(i_gen)].flatten(), dic['RF_pred_G0'+str(i_gen)].flatten()))
        else:
            print('Only one class, no ROC score.')
        print('--------------------------------------------------------------------------------------------------------')
    if i_gen >= 100:
        dic['RF_pred_G' + str(i_gen)] = dic['RF_G' + str(i_gen)].predict(Test_feature.T).T
        dic['RF_true_G' + str(i_gen)] = np.array(dic['Test_label_G' + str(i_gen)])
        print('--------------------------------------------------------------------------------------------------------')
        print('Scores for G',str(i_gen))
        print(classification_report(dic['RF_true_G' + str(i_gen)].flatten(), dic['RF_pred_G' + str(i_gen)].flatten()))
        if len(np.unique(dic['RF_true_G' + str(i_gen)])) > 1:
            print('ROC_score         ', roc_auc_score(dic['RF_true_G'+str(i_gen)].flatten(), dic['RF_pred_G'+str(i_gen)].flatten()))
        else:
            print('Only one class, no ROC score.')
        print('--------------------------------------------------------------------------------------------------------')