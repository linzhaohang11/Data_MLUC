...
for i_gen in Set_gen:
    if i_gen < 10:
        dic['DT_G00' + str(i_gen)] = tree.DecisionTreeClassifier(criterion='gini', splitter='best')
        dic['DT_GS_G00' + str(i_gen)] = GridSearchCV(dic['DT_G00' + str(i_gen)], param_grid, cv = 10, scoring = 'accuracy')
        dic['DT_GS_G00' + str(i_gen)].fit(Train_feature.values.T, dic['Train_label_G00' + str(i_gen)].values.T)
        dic['Best_Para_G00' + str(i_gen)] = dic['DT_GS_G00' + str(i_gen)].best_params_
        print('Parameters for DT_G00',str(i_gen), ':', dic['Best_Para_G00' + str(i_gen)])
    if 10 <= i_gen < 100:
...
# Use the best parameters to design DTs
for i_gen in Set_gen:
    if i_gen < 10:
        dic['DT_G00' + str(i_gen)] = tree.DecisionTreeClassifier(criterion         = 'gini',
                                                                 splitter          = 'best',
                                                                 max_depth         = dic['Best_Para_G00' + str(i_gen)]['max_depth'],
                                                                 min_samples_split = dic['Best_Para_G00' + str(i_gen)]['min_samples_split'],
                                                                 min_samples_leaf  = dic['Best_Para_G00' + str(i_gen)]['min_samples_leaf'],
                                                                 max_leaf_nodes    = dic['Best_Para_G00' + str(i_gen)]['max_leaf_nodes'])
        dic['DT_G00' + str(i_gen)] = dic['DT_G00' + str(i_gen)].fit(Train_feature.T, dic['Train_label_G00' + str(i_gen)].T)
        print(str(i_gen), dic['DT_G00' + str(i_gen)])
    if 10 <= i_gen < 100:
        ...
        print(str(i_gen), dic['DT_G0' + str(i_gen)])
    if i_gen >= 100:
        ...
        print(str(i_gen), dic['DT_G' + str(i_gen)])

# Check the scores
for i_gen in Set_gen:
    if i_gen < 10:
        dic['DT_pred_G00' + str(i_gen)] = dic['DT_G00' + str(i_gen)].predict(Test_feature.T).T
        dic['DT_true_G00' + str(i_gen)] = np.array(dic['Test_label_G00' + str(i_gen)])
        print('--------------------------------------------------------------------------------------------------------')
        print('Scores for G00',str(i_gen))
        print(classification_report(dic['DT_true_G00' + str(i_gen)].flatten(), dic['DT_pred_G00' + str(i_gen)].flatten()))
        if len(np.unique(dic['DT_true_G00' + str(i_gen)])) > 1:
            print('ROC_score         ', roc_auc_score(dic['DT_true_G00'+str(i_gen)].flatten(), dic['DT_pred_G00'+str(i_gen)].flatten()))
        else:
